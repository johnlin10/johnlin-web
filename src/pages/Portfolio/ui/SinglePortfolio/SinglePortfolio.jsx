...
function SinglePortfolio({ portfolioId, currentPortfolio }) {
  ...
  useEffect(() => {
    const hasWebVideo = currentPortfolio?.files.some(
      (file) => file.type === 'web_video'
    )
    const hasYouTubeVideo = currentPortfolio?.files.some((file) =>
  ...
  return (
    <>
      <Metadata
        title={`${currentPortfolio?.name || t('header.project')} | ${t(
          'header.project'
        )}`}
      ...
      >
        <div className={`${style.container} ${hasVideo ? style.hasVideo : ''}`}>
          <div className={style.mediaContainer}>
            {renderMedia(currentPortfolio, dispatch)}
          </div>
          <div className={style.mediaInfo}>
            ...
            <div className={style.details}>
              <div className={style.detailsItem}>
                ...
                <p>
                  {currentPortfolio?.authors?.join(', ') ||
                    t('detail.anonymous', { ns: 'project' })}
                </p>
              </div>
              <div className={style.detailsItem}>
                <p>{t('detail.startDate', { ns: 'project' })}</p>
                <p>{currentPortfolio?.startDate || '--'}</p>
              </div>
              <div className={style.detailsItem}>
                <p>{t('detail.endDate', { ns: 'project' })}</p>
                <p>{currentPortfolio?.endDate || '--'}</p>
              </div>
              <div className={style.detailsItem}>
                <p>{t('detail.tags', { ns: 'project' })}</p>
                <p>{currentPortfolio?.tags?.join(', ') || '--'}</p>
              </div>
              {currentPortfolio?.customFields?.map((field, index) => (
              ...
              <div className={style.detailsItem}>
                ...
                <p>
                  {currentPortfolio?.createdAt?.toDate().toLocaleDateString() ||
                    '--'}
                </p>
              </div>
            </div>
...
const renderMedia = (currentPortfolio, dispatch) => {
  // handle pdf click
  ...
  return currentPortfolio?.files.map((item, index) => {
    switch (item.type) {
      case 'youtube_video':
        return (
          <iframe
